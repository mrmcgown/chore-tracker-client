...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://chore-tracker.herokuapp.com/',
...
var choreEvents = __webpack_require__(11);
...
var onShowChores = function onShowChores(event) {
  event.preventDefault();
  // console.log('show chores ran!')
  $('#table').find('form.tr:has(span)').remove();

  api.showChores().then(ui.showChoresSuccess).then(addTableHandlers).catch(ui.showChoresFailure);

  $('span.td :input').attr('readonly', 'readonly');
};

var onAddChore = function onAddChore(event) {
  event.preventDefault();
  ...
  api.addChore(data).then(function () {
    return onShowChores(event);
  }).then(ui.addChoreSuccess).catch(ui.addChoreFailure);
};

var onDeleteChore = function onDeleteChore(event) {
  event.preventDefault();
  // console.log('delete chore ran!')
  var choreId = $(event.target).closest('form.tr').attr('data-id');

  // console.log(choreId)
  api.deleteChore(choreId).then(ui.deleteChoreSuccess).then(function () {
    return onShowChores(event);
...
var onSaveChore = function onSaveChore(event) {
  event.preventDefault();
  ...
  $(event.target).find('input').each(function (_, el) {
  ...
  };api.updateChore(data, choreId).then(ui.updateChoreSuccess).catch(ui.updateChoreFailure);
...
var addTableHandlers = function addTableHandlers() {
...
module.exports = {
  onShowChores: onShowChores,
  addHandlers: addHandlers,
  addTableHandlers: addTableHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var showChores = function showChores(data) {
  return $.ajax({
...
var addChore = function addChore(data) {
  return $.ajax({
    url: config.apiUrl + '/chores',
    method: 'POST',
    headers: {
...
var deleteChore = function deleteChore(choreId) {
  return $.ajax({
...
var updateChore = function updateChore(data, choreId) {
  return $.ajax({
    url: config.apiUrl + '/chores/' + choreId,
    method: 'PATCH',
    headers: {
...
var updateChoreFailure = function updateChoreFailure(error) {
...
module.exports = {
  showChoresSuccess: showChoresSuccess,
  showChoresFailure: showChoresFailure,
  addChoreSuccess: addChoreSuccess,
  addChoreFailure: addChoreFailure,
  deleteChoreSuccess: deleteChoreSuccess,
  deleteChoreFailure: deleteChoreFailure,
